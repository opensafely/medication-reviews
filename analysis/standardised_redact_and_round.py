...
for med_review in med_review_type:
    ...
    df.to_csv(OUTPUT_DIR / f"redacted/redacted_standardised_measure_{med_review}_population_rate.csv", index=False,)
    ...
    for breakdownby in breakdowns:
        ...
        df = redact_small_numbers(df, n=7, rounding_base=5, numerator=numerator_col, denominator="population", rate_column="value", date_column="date")  
        df.to_csv(OUTPUT_DIR / f"redacted/redacted_standardised_measure_{med_review}_{breakdownby}_rate.csv", index=False,)
